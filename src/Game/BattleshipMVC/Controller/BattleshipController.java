...
import Game.BattleshipMVC.Model.*;
import Game.BattleshipMVC.Model.Enums.GameStatus;
import Game.BattleshipMVC.Model.Enums.GridSquareStatus;
import Game.BattleshipMVC.View.MainPanel;
import Game.ClientServer.Client.Client;
...
import static Game.BattleshipMVC.Model.Enums.GameStatus.SERVER_LAYOUT;
...
public class BattleshipController {
    ...
    /**
     * Client that the controller is being created by, mainly used to re-enable
     * buttons after game ends and restarts
     */
     ...
     *
     * @param client Battleship client that will launch the controller
     */
    public void configure(Client client) {
        Locale.setDefault(new Locale("en", "CA"));
        this.client = client; // not ideal, but client needs to know when a new game happens
     ...
     *
     * @return true if game was successfully started, false if not
     */
    ...
    /**
     * Checks if the player's ship configuration is valid.
     *
     * @return True if ship configuration is valid, false otherwise
     */
    public boolean isShipConfigurationValid() {
        // random state and server layout configs are assumed to be valid by default, otherwise checks if all ships are placed
        GameStatus gameStatus = gameState.getStatus();
        return gameStatus == GameStatus.RANDOM || gameStatus == SERVER_LAYOUT || playerBoardState.isDesignBoatsEmpty();
    }
    ...
    /**
     * Returns the player's ship configuration as a String
     *
     * @return String containing details of the board's ship configuration
     */
    public String getShipConfiguration() {
        final StringBuilder shipConfiguration = new StringBuilder();
        final GameStatus gameStatus = gameState.getStatus();
        if (gameStatus == GameStatus.RANDOM || gameStatus == SERVER_LAYOUT || playerBoardState.isDesignBoatsEmpty()) {
            final String dimension = String.valueOf(playerBoardState.getGridDimension());
            shipConfiguration.append(dimension).append(Config.FIELD_SEPARATOR);
            for (GridBoat boat : playerBoardState.getBoatsOnBoard()) {
//              yes, the ship configuration is rather ugly with all the commas. this is because each square on the grid
//              belongs to a "ship" which has health. a message is displayed when the player sinks all the squares of a ship
//              and the ships are also used to calculate health. the config string being ugly is an easy workaround that
//              doesn't involve changing my game logic
                final int row = boat.getRow();
                final int column = boat.getColumn();
                final int size = boat.getBoatSize();
                final int isHorizontal = boat.isHorizontal() ? 1 : 0;
                shipConfiguration.append(row).append(Config.FIELD_SEPARATOR)
                        .append(column).append(Config.FIELD_SEPARATOR)
                        .append(size).append(Config.FIELD_SEPARATOR)
                        .append(isHorizontal).append(Config.FIELD_SEPARATOR);
            }
        }
        return shipConfiguration.toString();
    }

    /**
     * Reads and loads a boat configuration containing the dimension of the board and coordinates, size, and
     * orientation of boats on the board
     *
     * @param boatConfiguration String containing information about the board configuration
     */
    public void loadBoatConfiguration(String boatConfiguration) {
        final String[] config = boatConfiguration.split(Config.FIELD_SEPARATOR);
        final int dimension = Integer.parseInt(config[0]);
        changeBoardDimension(dimension);

        for (int i = 1; i < config.length; i += 4) {
            final int row = Integer.parseInt(config[i]);
            final int column = Integer.parseInt(config[i + 1]);
            final int boatSize = Integer.parseInt(config[i + 2]);
            final int isHorizontal = Integer.parseInt(config[i + 3]);

            playerBoardState.placeShipOnBoard(row, column, boatSize, isHorizontal == 1);
        }

        mainWindow.updateLogPanel("Boat layout successfully received and loaded from the server");
    }
}